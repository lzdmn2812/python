...
# черники высажены по окружности грядки. Каждый куст черники имеет урожайность, которая соответствует
...
# управляющего модуля и нескольких собирающих модулей. Каждый собирающий модуль может собрать ягоды 
# с одного куста и с двух соседних кустов. Собирающий модуль находится перед определенным кустом, и 
# он может выбирать, с какого куста начать сбор ягод.
...
# arr = [5, 8, 6, 4, 9, 2, 7, 3]
arr = [2, 4, 6, 8, 10]
max_numb_of_berries = arr[0]+arr[-1]+arr[1]
for i in range(1, len(arr)-1):
    if max_numb_of_berries < arr[i]+arr[i-1]+arr[i+1]:
        max_numb_of_berries = arr[i]+arr[i-1]+arr[i+1]
print(max_numb_of_berries)